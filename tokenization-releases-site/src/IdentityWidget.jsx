  ...
  return (
    <div className="relative">
      <button
        ...
        onClick={() => setOpen(o => !o)}
        onKeyDown={onKeyDown}
        className="w-full rounded-xl bg-[#0f162c] border border-white/10 px-3 py-2.5 text-left text-sm
                   hover:border-white/20 focus:outline-none focus:ring-2 focus:ring-sky-400/30
                   flex items-center justify-between gap-3"
        aria-haspopup="listbox"
        aria-expanded={open}
      >
        <span className="truncate">{current?.label}</span>
        <ChevronDown size={16} className={`transition-transform ${open ? 'rotate-180' : ''} text-white/60`} />
      </button>
      ...
      {open && (
        <ul
          ...
          role="listbox"
          tabIndex={-1}
          className="absolute z-10 mt-2 w-full rounded-xl border border-white/10 bg-[#0b1020]
                     shadow-2xl shadow-black/50 overflow-hidden ring-1 ring-white/10"
        >
          {options.map(opt => {
            ...
            return (
              <li
                ...
                className={`px-3 py-2.5 text-sm cursor-pointer flex items-center justify-between
                           hover:bg-white/5 ${active ? 'bg-white/[0.06]' : ''}`}
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <label className="block">
                      ...
                      <FancySelect
                        value={state.profile.citizenship}
